...
export const transformerWorkflowToDag = (
  ...
  const createDNode = (props: CreateDNodeProps): dNode => {
    const { compiledNode, parentDNode, taskTemplate, typeOverride, childrenNodes = [] } = props;
  ...
  const hasChildrenExecutions = (node: CompiledNode) => {
    if (
      Object.values(nodeExecutionsById).some(
        (nodeExecution) => nodeExecution.fromUniqueParentId === node.id,
      )
    ) {
      return true;
    }
  ...
  const transformWorkflowToCompiledNode = (dynamicWorkflow, node) => {
  ...
  const parseNode = ({ node, root }: ParseNodeProps) => {
    ...
    if (dynamicToMerge) {
      ...
      if (dynamicToMerge?.[id]) {
        ...
        if (dynamicWorkflow) {
          transformWorkflowToCompiledNode(dynamicWorkflow, node);
        }
    ...
    if (hasChildrenExecutions(node)) {
      const children = getNodeChildren(node);
      const childrenNodes: dNode[] = children.map((child) =>
        parseNodeFromExecution(child, (root?.level ?? 0) + 1),
      );
      dNode = createDNode({
        compiledNode: node,
        parentDNode: root,
   ...
   *
   * @param node  Node for which list will be built
   */
  const getNodeChildren = (node: CompiledNode) => {
   ...
   */
  const buildDAG = (root, context: any, graphType: dTypes) => {
    switch (graphType) {
      case dTypes.branch:
