      ...
      return {
        executions: data.map(executionId =>
          Admin.Execution.create(
            getItem(entityMap, [EntityType.WorkflowExecution, executionId]),
          ),
        ),
      };
    },
        ...
        taskExecutions: data.map(taskExecutionId => {
          try {
            ...
            throw unexpectedError(
              `Unexpected missing child item: ${obj(taskExecutionId)}`,
            );
          }
        }),
      };
      ...
      return {
        nodeExecutions: data.map(nodeExecutionId => {
          try {
            ...
            throw unexpectedError(
              `Unexpected missing child item: ${obj(nodeExecutionId)}`,
            );
          }
        }),
      };
