...
describe('models/Task', () => {
  ...
  describe('isMapTaskV1 propely identifies mapped task events above version 1', () => {
    ...
    describe('FALSE cases:', () => {
      ...
      expect(
        isMapTaskV1(eventVersionN, resourcesLengthN, nonMapTaskType),
      ).toBeFalsy();
      // when eventVersion < 1
      expect(
        isMapTaskV1(eventVersionZero, resourcesLengthN, mapTaskTypes[0]),
      ).toBeFalsy();
      // when externalResources array has length of 0
      expect(
        isMapTaskV1(eventVersionN, resourcesLengthZero, mapTaskTypes[0]),
      ).toBeFalsy();
    });
