...
export function timestampToDate(timestamp: Protobuf.ITimestamp): Date {
  ...
  const milliseconds =
    Long.fromValue(timestamp?.seconds!).toNumber() * 1000 + nanos / 1e6;
...
export function compareTimestampsAscending(
  ...
  const leftSeconds: Long =
    (typeof a.seconds === 'number' ? Long.fromNumber(a.seconds) : a.seconds) ||
    Long.fromNumber(0);
  const leftNanos: number = a.nanos || 0;
  // i was here
  const rightSeconds: Long =
    (typeof b.seconds === 'number' ? Long.fromNumber(b.seconds) : b.seconds) ||
    Long.fromNumber(0);
