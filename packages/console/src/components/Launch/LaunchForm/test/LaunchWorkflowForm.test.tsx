...
describe('LaunchForm: Workflow', () => {
  ...
  let mockCreateWorkflowExecution: jest.Mock<
    ReturnType<typeof createWorkflowExecution>
  >;
  ...
  describe('With Simple Inputs', () => {
    ...
    describe('Input Values', () => {
      ...
      it('should use default values when provided', async () => {
        ...
        expect(getByLabelText(stringInputName, { exact: false })).toHaveValue(
          'abc',
        );
        expect(getByLabelText(integerInputName, { exact: false })).toHaveValue(
          '10000',
        );
      });
    ...
    describe('When using initial parameters', () => {
      ...
      it('should only include one instance of the preferred version in the selector', async () => {
        ...
        expect(
          items.filter(
            item =>
              item.textContent && item.textContent.includes(expectedVersion),
          ),
        ).toHaveLength(1);
      });
      ...
      it('should prefer the provided launch plan', async () => {
        ...
        const { getByLabelText } = renderForm({ initialParameters });
        await waitFor(() => {});
        expect(getByLabelText(t('launchPlan'))).toHaveValue(
      ...
      it('should only include one instance of the preferred launch plan in the selector', async () => {
        ...
        expect(
          items.filter(
            item => item.textContent && item.textContent.includes(expectedName),
          ),
        ).toHaveLength(1);
      });
      ...
      it('should maintain selected launch plan by name after switching workflow versions', async () => {
        ...
        await fireEvent.click(launchPlanExpander);
        const launchPlanItems = await waitFor(() =>
          getAllByRole(launchPlanDiv, 'menuitem'),
        );
        await fireEvent.click(launchPlanItems[1]);
        await waitFor(() => {});
    ...
    describe('With Unsupported Required Inputs', () => {
      ...
      it('should render error message', async () => {
        ...
        const errorElement = await waitFor(() =>
          getByText(t('cannotLaunchWorkflowString')),
        );
