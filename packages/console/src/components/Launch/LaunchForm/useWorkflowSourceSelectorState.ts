...
function useLaunchPlanSelectorOptions(launchPlans: LaunchPlan[]) {
  return useMemo(
    () => launchPlansToSearchableSelectorOptions(launchPlans),
    [launchPlans],
  );
}
...
export function useWorkflowSourceSelectorState({
  ...
  const [workflowVersionSearchOptions, setWorkflowVersionSearchOptions] =
    useState<SearchableSelectorOption<WorkflowId>[]>([]);
  const launchPlanSelectorOptions =
    useLaunchPlanSelectorOptions(launchPlanOptions);
