...
jest.mock(
  ...
  () => ({
    NodeExecutionName: jest.fn(({ children }) => (
      <div data-testid="task-names">{children}</div>
    )),
  }),
);
...
describe('flytegraph > ReactFlow > PausedTasksComponent', () => {
  ...
  it('should render Paused Tasks table, if initialIsVisible is true', () => {
    ...
    expect(queryAllByTestId('task-name-item').length).toEqual(
      pausedNodes.length,
    );
  });
  ...
  it('should render LaunchFormDialog on resume button click', async () => {
    const { getByRole, queryByTitle, queryByTestId, getByTestId } =
      renderComponent({
        pausedNodes,
      });
    expect(queryByTitle('Paused Tasks')).toBeInTheDocument();
    ...
    expect(queryByTestId('paused-tasks-table')).toBeInTheDocument();
    expect(
      queryByTestId(`resume-gate-node-${pausedNodes[0].id}`),
    ).toBeInTheDocument();
