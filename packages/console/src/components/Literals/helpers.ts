...
function processPrimitive(
  primitive?: (Core.IPrimitive & Pick<Core.Primitive, 'value'>) | null,
) {
  if (!primitive) {
  ...
  switch (type) {
    case 'datetime':
      return formatDateUTC(
        timestampToDate(primitive.datetime as Protobuf.Timestamp),
      );
    case 'duration':
...
function processSchemaType(
  schemaType?: Core.ISchemaType | null,
  shortString = false,
) {
  const columns =
...
function processGenericValue(
  value: Protobuf.IValue & Pick<Protobuf.Value, 'kind'>,
) {
...
function processStructuredDatasetType(
  structuredDatasetType?: Core.IStructuredDatasetType | null,
) {
...
function processStructuredDataset(
  structuredDataSet?: Core.IStructuredDataset | null,
) {
  ...
  const structuredDatasetType = processStructuredDatasetType(
    metadata?.structuredDatasetType,
  );
...
function processScalar(
  scalar?: (Core.IScalar & Pick<Core.Scalar, 'value'>) | null,
) {
