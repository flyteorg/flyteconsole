...
import {
  ...
  extractSchemaTypes,
  extractBlobTypes,
} from './helpers/typeGenerators';

const stories = storiesOf('Literals/StructuredDataSet', module);

function generateStructuredDataset(
  label: string,
  literalType: Core.ILiteralType,
) {
  return {
    literals: {
      [label]: {
...
stories.add('Collection Type', () => (
  <>{renderScalars(generateCollectionTypes())}</>
));
stories.add('mapValue Type', () => (
  <>{renderScalars(generateMapValueypes())}</>
));
