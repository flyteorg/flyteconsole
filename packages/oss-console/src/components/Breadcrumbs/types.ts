 ...
 */
export type BreadcrumbValidator = (validator: BreadcrumbValidatorInterface) => boolean;
 ...
 */
export type BreadcrumbCustomDefaultValue = (location: Location, breadcrumb: Breadcrumb) => string;
...
export interface BreadcrumbEntity {
  ...
  title: string;
  createdAt: string;
  id?: string; // used where entity name and id are different
  active?: boolean;
}
...
export type BreadcrumbEntitySelfLink =
  | string
  | ((location: Location, breadcrumb: BreadcrumbFormControlInterface) => string);
...
export interface BreadcrumbPopoverInterface extends BreadcrumbFormControlInterface {
  open: boolean;
