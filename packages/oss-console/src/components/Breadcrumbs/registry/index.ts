...
import { Breadcrumb, BreadcrumbFormControlInterface, BreadcrumbValidatorInterface } from '../types';
...
export class BreadcrumbRegistry {
  ...
  private _makeRenderHash() {
    this.renderHash = `${this.breadcrumbs
      .map((b) => {
        return Object.keys(b)
          .map((k) => b[k])
          .join(',');
      })
      .join(',')}|${this.breadcrumbSeeds
      .map((b) => {
        return Object.keys(b)
          .map((k) => b[k])
          .join(',');
  ...
  public addBreadcrumbSeed(breadcrumb: Partial<Breadcrumb>) {
    ...
    const existingBreadcrumbIndex = this.breadcrumbSeeds.findIndex((b) => b.id === breadcrumb.id);

    if (existingBreadcrumbIndex > -1) {
      const existingBreadcrumb = this.breadcrumbSeeds[existingBreadcrumbIndex];

      const newBreadcrumb = mergeWith(existingBreadcrumb, breadcrumbData, (exVal, newVal) => {
  ...
  static makeUrlSegments(location: Location, projectId = '', domainId = '') {
    ...
    for (let i = 0; i < pathFragments.length; i += 2) {
    ...
    breadcrumbRegistry.breadcrumbSeeds
      ...
      .forEach((b) => {
        if (!values[b.id]) {
          const value =
            typeof b.defaultValue === 'function' ? b.defaultValue(location, b) : b.defaultValue;
          values[b.id] = value || '';
        }
      });
  ...
  public breadcrumbBuilder({
    ...
    for (let i = 0; i < validSeeds.length; i++) {
      ...
      const controller: BreadcrumbFormControlInterface = {
        ...breadcrumb,
        value,
        key: `breadcrumb-controller-${breadcrumb.id}-${value || breadcrumb.defaultValue}`,
        projectId,
        domainId,
      };
