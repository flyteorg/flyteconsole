...
export const taskSelfLink = (_location: Location, breadcrumb: BreadcrumbFormControlInterface) => {
  const { projectId, domainId, value } = breadcrumb;
...
export const namedEntitiesSelfLink = async (
  ...
  const entity = namedEntities.find((entity) => camelCase(entity.title).includes(key));
  return entity?.url || '';
};
