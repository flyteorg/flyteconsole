...
/** Determines if a workflow is in unknown, queued or running state.
 */
export const executionIsStoppable = (execution: Execution) =>
  execution.closure && execution.closure.phase <= WorkflowExecutionPhase.RUNNING;
...
function getExecutionTimingMS({
  ...
  isTerminal,
}: GetExecutionDurationMSArgs): GetExecutionTimingMSResult | undefined {
...
/** Indicates if a NodeExecution is explicitly marked as a parent node. */
export function isDynamicNode(nodeExecution: NodeExecution): nodeExecution is DynamicNodeExecution {
