...
import { FilterOperationName, FilterOperationValue } from '@clients/common/types/adminEntityTypes';
...
>({
  ...
  const parsedQueryState = useMemo(
    () => deserializeFromQueryState(queryStateValue),
    [queryStateValue],
  );
  ...
  const selectedKeys = useMemo(() => Object.keys(pickBy(selected)), [selected]);
  const active = useMemo(() => !isEqual(selectedKeys, defaultValue), [selectedKeys, defaultValue]);
