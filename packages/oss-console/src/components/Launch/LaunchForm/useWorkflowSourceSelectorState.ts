...
function generateFetchSearchResults(
  ...
  return async (workflowVersionId: string) => {
...
export function useWorkflowSourceSelectorState({
  ...
  const selectedWorkflow = useMemo(() => {
    ...
    return [...workflowSelectorOptions, ...workflowVersionSearchOptions].find((option) =>
      stringifyIsEqual(option.id, workflowVersion.version),
    );
  ...
  const fetchSearchResults = useMemo(() => {
    ...
    return async (workflowVersionId: string) => {
      const results = await doFetch(workflowVersionId);
