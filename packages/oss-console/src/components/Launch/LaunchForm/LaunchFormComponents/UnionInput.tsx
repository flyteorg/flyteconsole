...
import { formatType, getLaunchInputId } from '../utils';
...
const generateInputTypeToValueMap = (
  ...
  const final = listOfSubTypes.reduce((map, subType) => {
    let finalValue: UnionValue;
    if (initialInputValue && subType.type === initialType.type) {
      ...
      finalValue = {
        value: subtypeHelper?.typeDefinitionToDefaultValue?.(subType) || undefined,
        typeDefinition: subType,
      } as UnionValue;
    }
    return { ...map, [subType.type]: finalValue };
  }, {});
  return final;
};
...
const getInitialInputValue = (props: InputProps): UnionValue => {
  ...
  if (props.hasCollectionParent && Array.isArray(props.initialValue)) {
    const collectionValues = props.initialValue.map((literal) => {
      const unionValue = unionHelper.fromLiteral(literal, props.typeDefinition) as UnionValue;

      return unionValue;
    });

    const subtype =
      collectionValues?.[0]?.typeDefinition || props.typeDefinition.listOfSubTypes?.[0];
  ...
  return (
    props.initialValue &&
    (unionHelper.fromLiteral(props.initialValue, props.typeDefinition) as UnionValue as any)
  );
};
...
export const UnionInput = (props: InputProps) => {
  const { label, name, onChange, typeDefinition, error, hasCollectionParent } = props;
  ...
  return (
    <StyledCard error={error} label={label} name={name}>
      <Selector
        ...
        selectedItem={generateSearchableSelectorOption(selectedInputTypeDefinition)}
