...
export const CollectionInput: FC<InputProps> = (props) => {
  const { typeDefinition, value, onChange, error, label, name } = props;
  ...
  const { isTextSubType, newprops, subtypeDefaultValue } = useMemo(() => {
    ...
    const subtypeDefaultValue = subtypeHelper.typeDefinitionToDefaultValue(subtype!);
    const isTextSubType =
    ...
    const finalValue = isTextSubType
      ? value
      : tryGetCollectionValue({ value, typeDefinition } as any, typeDefinition, helper) || [
          subtypeDefaultValue,
        ];
  ...
  const updateCollection = (inputs: InputValue[]) => {
    ...
    try {
      ...
      collectionString = helper.fromLiteral(collectionLiteral, typeDefinition) as any;
    } catch (error) {
