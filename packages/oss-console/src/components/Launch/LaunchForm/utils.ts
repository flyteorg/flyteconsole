...
import { primitiveToInputType, simpleTypeToInputType, typeLabels } from './constants';
...
export function getInputDefintionForLiteralType(
  ...
  if (literalType.noneType) {
    ...
    const { primitive } = literalType as any;

    result.type = primitiveToInputType[primitive.value || Object.keys(primitive || {})?.[0]];
  }
  return result;
}

export function getLaunchInputId(name: string): string {
  return `launch-input-${name.replaceAll(/ /g, '-')}`;
}
...
export function isBlobValue(value: any): value is BlobValue {
  return isObject(value) && ('uri' in value || 'format' in value || 'dimensionality' in value);
}
