...
function fromLiteral(literal: Core.ILiteral, { subtype }: InputTypeDefinition): string {
  if (!subtype) {
...
function validate({ value, typeDefinition, required, ...props }: InputValidatorParams) {
  ...
  try {
    ...
    if (!Array.isArray(parsed)) {
      throw new Error(`Value parsed to type: ${typeof parsed}. Expected format: ${typeString}`);
    }
    // validate sub values
    ...
    const typeString = formatType(typeDefinition);
    throw new Error(`Failed to parse to expected format: ${typeString}. ${e.message}`);
  }
}
...
export const collectionHelper: InputHelper = {
  ...
  typeDefinitionToDefaultValue: (typeDefinition) => {
    const { subtype } = typeDefinition;
    const subtypeHelper = getHelperForInput(subtype?.type!);
    const subDefaultValue = subtypeHelper.typeDefinitionToDefaultValue(subtype!);
    const subLiteral = subtypeHelper.toLiteral({
      value: subDefaultValue,
      typeDefinition: subtype!,
    });
    let literalArray: Core.ILiteral[] | undefined;
    if (subDefaultValue !== undefined && subDefaultValue !== null && subDefaultValue !== '') {
