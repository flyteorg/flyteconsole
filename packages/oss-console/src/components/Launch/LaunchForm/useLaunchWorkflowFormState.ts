...
async function loadLaunchPlans(
  ...
  if (workflowVersion == null) {
    return Promise.reject(new Error('No workflowVersion specified'));
  }
  ...
  useEffect(() => {
    const subscription = service.subscribe((newState) => {
      if (newState.matches(LaunchState.SELECT_WORKFLOW_VERSION)) {
        const { workflowVersionOptions, preferredWorkflowId, workflowVersion } =
        ...
        if (workflowVersionOptions.length > 0) {
          ...
          const finalPreferredWorkflowId = preferredWorkflowId || workflowVersion;
          if (finalPreferredWorkflowId) {
            const preferred = workflowVersionOptions.find(({ id }) => {
              const ismatch = stringifyIsEqual(id, finalPreferredWorkflowId);
              return ismatch;
            });
            if (preferred) {
