...
const TopLevelLayoutProvider = ({ children }: PropsWithChildren) => {
  ...
  const [isLayoutHorizontal, setisLayoutHorizontal] = useState(initValues.isLayoutHorizontal);

  const openSideNav = useCallback(() => setIsSideNavOpen(true), [isSideNavOpen, setIsSideNavOpen]);
  ...
  const showMobileNav = useCallback(() => setIsMobileNav(true), [isMobileNav, setIsMobileNav]);
  const hideMobileNav = useCallback(() => setIsMobileNav(false), [isMobileNav, setIsMobileNav]);
