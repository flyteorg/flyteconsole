...
import { EncodableType } from '@clients/common/types/adminEntityTypes';
...
type RestRequest = MockedRequest<Identifier>;
type RestResolver = ResponseResolver<MockedRequest<Identifier>, RestContext, any>;
...
export function createAdminServer(): CreateAdminServerResult {
  ...
  const getWorkflowHandler = adminEntityHandler({
    ...
    getDataForRequest: (req) => {
      const [, project, domain, name, version] = req.url.pathname.split('/');
  ...
  const getWorkflowExecutionListHandler = adminEntityHandler({
    ...
    getDataForRequest: (req) => {
      const [, project, domain] = req.url.pathname.split('/');
  ...
  const getWorkflowExecutionHandler = adminEntityHandler({
    ...
    getDataForRequest: (req) => {
      const [, project, domain, name] = req.url.pathname.split('/');
  ...
  const getNodeExecutionHandler = adminEntityHandler({
    ...
    getDataForRequest: (req) => {
      const [, project, domain, name, nodeId] = req.url.pathname.split('/');
  ...
  const getTaskExecutionHandler = adminEntityHandler({
    ...
    getDataForRequest: (req) => {
      ...
      const id = taskExecutionIdFromParams(params);
